...
#define OFF(x, y, ch, bc) ((y) * pitch + (x) * bc + ch)
__kernel void Fourier(__global uchar *in, __global uchar *out, int width, int height, int pitch, int bitcount)
{
	...
	if (mag < 0.0)
		mag = 0.0;
	dst[OFF(u, v, 0, bitcount)] = mag;
	dst[OFF(u, v, 1, bitcount)] = mag;
	dst[OFF(u, v, 2, bitcount)] = mag;
}